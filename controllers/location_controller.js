...
async function createLocation(req, res) {
  ...
  try {
    ...
    if (findLocationByName) {
      throw new Error(
        "validationError: локация с таким названием уже существует"
      );
    }
    const findLocationByNumber = await db.Location.findOne({
      where: { number: req.body.number }
    });
    console.log("findLocationByNumber = ", findLocationByNumber);
    if (findLocationByNumber) {
      throw new Error(
        "validationError: локация с таким номером уже существует"
      );
    }
...
async function updateLocation(req, res) {
  ...
  try {
    ...
    if (req.body.name && location.name != req.body.name) {
      ...
      if (location.name !== req.body.name && findLocationByName) {
        throw new Error(
    ...
    if (req.body.number && location.number != req.body.number) {
      ...
      if (location.number !== req.body.number && findLocationByNumber) {
...
async function deleteLocation(req, res) {
  ...
  try {
    const location = await db.Location.findByPk(req.params.id);
    if (!location) {
      throw new Error("validationError: Location by this id is not found!");
    }
    await location.destroy();
    res.json({ massage: `location with id ${location.id} is deleted` });
  } catch (error) {
    console.error(error);
